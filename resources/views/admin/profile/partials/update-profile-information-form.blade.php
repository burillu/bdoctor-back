    ...
    <form method="post" action="{{ route('admin.profile.update', $data->id) }}" class="mt-6 space-y-6"
        ...
        <div class="mb-2">
            ...
            <div>
                ...
                @enderror
                @if ($data->curriculum)
                <a class="btn btn-primary my-2" href="{{ asset('storage/' . $data->curriculum) }}" download>Scarica
                    PDF</a>
                @endif
            </div>
            ...
            <input id="email" name="email" type="" class="form-control @error('email') is-invalid @enderror"
        ...
        <div class="mb-2">
            ...
            <input class="form-control @error('address') is-invalid @enderror" type="text" name="address" id="address"
                autocomplete="address" value="{{ old('address', $data->address) }}" autofocus>
            @error('address')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
